...
func (dst *NiatelemetryNiaInventoryRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "niatelemetry.NiaInventory":
			var result *NiatelemetryNiaInventory = &NiatelemetryNiaInventory{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryNiaInventory = result
			return nil
		default:
...
func (src *NiatelemetryNiaInventoryRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.NiatelemetryNiaInventory != nil {
		return yaml.Marshal(&src.NiatelemetryNiaInventory)
	}
