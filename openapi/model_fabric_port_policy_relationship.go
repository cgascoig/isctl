...
func (dst *FabricPortPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "fabric.PortPolicy":
			var result *FabricPortPolicy = &FabricPortPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.FabricPortPolicy = result
			return nil
		case "mo.MoRef":
...
func (src *FabricPortPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.FabricPortPolicy != nil {
		return yaml.Marshal(&src.FabricPortPolicy)
	}
