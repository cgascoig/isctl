...
func (dst *SoftwarerepositoryFileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "softwarerepository.File":
			var result *SoftwarerepositoryFile = &SoftwarerepositoryFile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SoftwarerepositoryFile = result
			return nil
		default:
...
func (src *SoftwarerepositoryFileRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.SoftwarerepositoryFile != nil {
		return yaml.Marshal(&src.SoftwarerepositoryFile)
	}
