...
func (dst *UuidpoolBlockResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "uuidpool.Block.List":
			var result *UuidpoolBlockList = &UuidpoolBlockList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.UuidpoolBlockList = result
			return nil
		default:
...
func (src *UuidpoolBlockResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.UuidpoolBlockList != nil {
		return yaml.Marshal(&src.UuidpoolBlockList)
	}
