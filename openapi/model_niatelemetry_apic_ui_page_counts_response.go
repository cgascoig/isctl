...
func (dst *NiatelemetryApicUiPageCountsResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.ApicUiPageCounts.List":
			var result *NiatelemetryApicUiPageCountsList = &NiatelemetryApicUiPageCountsList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryApicUiPageCountsList = result
			return nil
		default:
...
func (src *NiatelemetryApicUiPageCountsResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryApicUiPageCountsList != nil {
		return yaml.Marshal(&src.NiatelemetryApicUiPageCountsList)
	}
