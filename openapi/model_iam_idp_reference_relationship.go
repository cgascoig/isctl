...
func (dst *IamIdpReferenceRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.IdpReference":
			var result *IamIdpReference = &IamIdpReference{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamIdpReference = result
			return nil
		case "mo.MoRef":
...
func (src *IamIdpReferenceRelationship) MarshalYAML() ([]byte, error) {
	if src.IamIdpReference != nil {
		return yaml.Marshal(&src.IamIdpReference)
	}
