...
func (dst *NiatelemetryMsoSiteDetailsResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.MsoSiteDetails.List":
			var result *NiatelemetryMsoSiteDetailsList = &NiatelemetryMsoSiteDetailsList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryMsoSiteDetailsList = result
			return nil
		default:
...
func (src *NiatelemetryMsoSiteDetailsResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryMsoSiteDetailsList != nil {
		return yaml.Marshal(&src.NiatelemetryMsoSiteDetailsList)
	}
