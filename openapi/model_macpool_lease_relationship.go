...
func (dst *MacpoolLeaseRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "macpool.Lease":
			var result *MacpoolLease = &MacpoolLease{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.MacpoolLease = result
			return nil
		case "mo.MoRef":
...
func (src *MacpoolLeaseRelationship) MarshalYAML() ([]byte, error) {
	if src.MacpoolLease != nil {
		return yaml.Marshal(&src.MacpoolLease)
	}
