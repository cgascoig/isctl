...
func (dst *NiatelemetryFaultResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.Fault.List":
			var result *NiatelemetryFaultList = &NiatelemetryFaultList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryFaultList = result
			return nil
		default:
...
func (src *NiatelemetryFaultResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryFaultList != nil {
		return yaml.Marshal(&src.NiatelemetryFaultList)
	}
