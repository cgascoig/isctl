...
func (dst *VirtualizationVmwareHostResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "virtualization.VmwareHost.List":
			var result *VirtualizationVmwareHostList = &VirtualizationVmwareHostList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VirtualizationVmwareHostList = result
			return nil
		default:
...
func (src *VirtualizationVmwareHostResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.VirtualizationVmwareHostList != nil {
		return yaml.Marshal(&src.VirtualizationVmwareHostList)
	}
