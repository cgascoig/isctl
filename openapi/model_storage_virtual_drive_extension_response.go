...
func (dst *StorageVirtualDriveExtensionResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.VirtualDriveExtension.List":
			var result *StorageVirtualDriveExtensionList = &StorageVirtualDriveExtensionList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageVirtualDriveExtensionList = result
			return nil
		default:
...
func (src *StorageVirtualDriveExtensionResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageVirtualDriveExtensionList != nil {
		return yaml.Marshal(&src.StorageVirtualDriveExtensionList)
	}
