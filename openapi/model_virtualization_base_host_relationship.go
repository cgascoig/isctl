...
func (dst *VirtualizationBaseHostRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "virtualization.BaseHost":
			var result *VirtualizationBaseHost = &VirtualizationBaseHost{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VirtualizationBaseHost = result
			return nil
		default:
...
func (src *VirtualizationBaseHostRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VirtualizationBaseHost != nil {
		return yaml.Marshal(&src.VirtualizationBaseHost)
	}
