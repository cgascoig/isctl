...
func (dst *ServerConfigImportResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "server.ConfigImport.List":
			var result *ServerConfigImportList = &ServerConfigImportList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ServerConfigImportList = result
			return nil
		default:
...
func (src *ServerConfigImportResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.ServerConfigImportList != nil {
		return yaml.Marshal(&src.ServerConfigImportList)
	}
