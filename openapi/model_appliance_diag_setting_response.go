...
func (dst *ApplianceDiagSettingResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		switch v {
		case "appliance.DiagSetting.List":
			var result *ApplianceDiagSettingList = &ApplianceDiagSettingList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ApplianceDiagSettingList = result
			return nil
...
func (src *ApplianceDiagSettingResponse) MarshalYAML() ([]byte, error) {
	if src.ApplianceDiagSettingList != nil {
		return yaml.Marshal(&src.ApplianceDiagSettingList)
	}
