...
func (dst *RecoveryConfigResultEntryResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "recovery.ConfigResultEntry.List":
			var result *RecoveryConfigResultEntryList = &RecoveryConfigResultEntryList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.RecoveryConfigResultEntryList = result
			return nil
		default:
...
func (src *RecoveryConfigResultEntryResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.RecoveryConfigResultEntryList != nil {
		return yaml.Marshal(&src.RecoveryConfigResultEntryList)
	}
