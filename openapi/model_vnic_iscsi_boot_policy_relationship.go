...
func (dst *VnicIscsiBootPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "vnic.IscsiBootPolicy":
			var result *VnicIscsiBootPolicy = &VnicIscsiBootPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicIscsiBootPolicy = result
			return nil
		default:
...
func (src *VnicIscsiBootPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicIscsiBootPolicy != nil {
		return yaml.Marshal(&src.VnicIscsiBootPolicy)
	}
