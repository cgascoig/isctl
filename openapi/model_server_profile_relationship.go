...
func (dst *ServerProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			return nil
		case "server.Profile":
			var result *ServerProfile = &ServerProfile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ServerProfile = result
			return nil
		default:
...
func (src *ServerProfileRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.ServerProfile != nil {
		return yaml.Marshal(&src.ServerProfile)
	}
