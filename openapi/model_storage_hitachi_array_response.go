...
func (dst *StorageHitachiArrayResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.HitachiArray.List":
			var result *StorageHitachiArrayList = &StorageHitachiArrayList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHitachiArrayList = result
			return nil
		default:
...
func (src *StorageHitachiArrayResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageHitachiArrayList != nil {
		return yaml.Marshal(&src.StorageHitachiArrayList)
	}
