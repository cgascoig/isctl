...
func (dst *WorkflowTaskDebugLogResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "workflow.TaskDebugLog.List":
			var result *WorkflowTaskDebugLogList = &WorkflowTaskDebugLogList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowTaskDebugLogList = result
			return nil
		default:
...
func (src *WorkflowTaskDebugLogResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowTaskDebugLogList != nil {
		return yaml.Marshal(&src.WorkflowTaskDebugLogList)
	}
