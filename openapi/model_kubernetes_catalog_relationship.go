...
func (dst *KubernetesCatalogRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "kubernetes.Catalog":
			var result *KubernetesCatalog = &KubernetesCatalog{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.KubernetesCatalog = result
			return nil
		case "mo.MoRef":
...
func (src *KubernetesCatalogRelationship) MarshalYAML() ([]byte, error) {
	if src.KubernetesCatalog != nil {
		return yaml.Marshal(&src.KubernetesCatalog)
	}
