...
func (dst *OsInstallResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "os.Install.List":
			var result *OsInstallList = &OsInstallList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.OsInstallList = result
			return nil
		default:
...
func (src *OsInstallResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.OsInstallList != nil {
		return yaml.Marshal(&src.OsInstallList)
	}
