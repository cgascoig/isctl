...
func (dst *ForecastDefinitionRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "forecast.Definition":
			var result *ForecastDefinition = &ForecastDefinition{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ForecastDefinition = result
			return nil
		case "mo.MoRef":
...
func (src *ForecastDefinitionRelationship) MarshalYAML() ([]byte, error) {
	if src.ForecastDefinition != nil {
		return yaml.Marshal(&src.ForecastDefinition)
	}
