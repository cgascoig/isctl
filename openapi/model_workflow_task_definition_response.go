...
func (dst *WorkflowTaskDefinitionResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "workflow.TaskDefinition.List":
			var result *WorkflowTaskDefinitionList = &WorkflowTaskDefinitionList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowTaskDefinitionList = result
			return nil
		default:
...
func (src *WorkflowTaskDefinitionResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowTaskDefinitionList != nil {
		return yaml.Marshal(&src.WorkflowTaskDefinitionList)
	}
