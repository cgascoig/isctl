...
func (dst *ManagementEntityRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "management.Entity":
			var result *ManagementEntity = &ManagementEntity{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ManagementEntity = result
			return nil
		case "mo.MoRef":
...
func (src *ManagementEntityRelationship) MarshalYAML() ([]byte, error) {
	if src.ManagementEntity != nil {
		return yaml.Marshal(&src.ManagementEntity)
	}
