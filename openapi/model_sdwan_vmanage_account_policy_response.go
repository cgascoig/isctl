...
func (dst *SdwanVmanageAccountPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "sdwan.VmanageAccountPolicy.List":
			var result *SdwanVmanageAccountPolicyList = &SdwanVmanageAccountPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SdwanVmanageAccountPolicyList = result
			return nil
		default:
...
func (src *SdwanVmanageAccountPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.SdwanVmanageAccountPolicyList != nil {
		return yaml.Marshal(&src.SdwanVmanageAccountPolicyList)
	}
