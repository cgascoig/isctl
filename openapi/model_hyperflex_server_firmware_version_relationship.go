...
func (dst *HyperflexServerFirmwareVersionRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hyperflex.ServerFirmwareVersion":
			var result *HyperflexServerFirmwareVersion = &HyperflexServerFirmwareVersion{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexServerFirmwareVersion = result
			return nil
		case "mo.MoRef":
...
func (src *HyperflexServerFirmwareVersionRelationship) MarshalYAML() ([]byte, error) {
	if src.HyperflexServerFirmwareVersion != nil {
		return yaml.Marshal(&src.HyperflexServerFirmwareVersion)
	}
