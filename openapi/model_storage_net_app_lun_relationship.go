...
func (dst *StorageNetAppLunRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.NetAppLun":
			var result *StorageNetAppLun = &StorageNetAppLun{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppLun = result
			return nil
		default:
...
func (src *StorageNetAppLunRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageNetAppLun != nil {
		return yaml.Marshal(&src.StorageNetAppLun)
	}
