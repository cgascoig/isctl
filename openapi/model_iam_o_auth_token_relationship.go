...
func (dst *IamOAuthTokenRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.OAuthToken":
			var result *IamOAuthToken = &IamOAuthToken{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamOAuthToken = result
			return nil
		case "mo.MoRef":
...
func (src *IamOAuthTokenRelationship) MarshalYAML() ([]byte, error) {
	if src.IamOAuthToken != nil {
		return yaml.Marshal(&src.IamOAuthToken)
	}
