...
func (dst *SshPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "ssh.Policy.List":
			var result *SshPolicyList = &SshPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SshPolicyList = result
			return nil
		default:
...
func (src *SshPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.SshPolicyList != nil {
		return yaml.Marshal(&src.SshPolicyList)
	}
