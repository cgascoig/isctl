...
func (dst *StoragePureControllerRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.PureController":
			var result *StoragePureController = &StoragePureController{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureController = result
			return nil
		default:
...
func (src *StoragePureControllerRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StoragePureController != nil {
		return yaml.Marshal(&src.StoragePureController)
	}
