...
func (dst *IamLdapProviderRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.LdapProvider":
			var result *IamLdapProvider = &IamLdapProvider{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamLdapProvider = result
			return nil
		case "mo.MoRef":
...
func (src *IamLdapProviderRelationship) MarshalYAML() ([]byte, error) {
	if src.IamLdapProvider != nil {
		return yaml.Marshal(&src.IamLdapProvider)
	}
