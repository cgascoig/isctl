...
func (dst *TamAdvisoryInstanceResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "tam.AdvisoryInstance.List":
			var result *TamAdvisoryInstanceList = &TamAdvisoryInstanceList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.TamAdvisoryInstanceList = result
			return nil
		default:
...
func (src *TamAdvisoryInstanceResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.TamAdvisoryInstanceList != nil {
		return yaml.Marshal(&src.TamAdvisoryInstanceList)
	}
