...
func (dst *ChassisIomProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "chassis.IomProfile":
			var result *ChassisIomProfile = &ChassisIomProfile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ChassisIomProfile = result
			return nil
		case "mo.MoRef":
...
func (src *ChassisIomProfileRelationship) MarshalYAML() ([]byte, error) {
	if src.ChassisIomProfile != nil {
		return yaml.Marshal(&src.ChassisIomProfile)
	}
