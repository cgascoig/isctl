...
func (dst *VnicLcpStatusResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "vnic.LcpStatus.List":
			var result *VnicLcpStatusList = &VnicLcpStatusList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicLcpStatusList = result
			return nil
		default:
...
func (src *VnicLcpStatusResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VnicLcpStatusList != nil {
		return yaml.Marshal(&src.VnicLcpStatusList)
	}
