...
func (dst *WorkflowTaskDefinitionRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "workflow.TaskDefinition":
			var result *WorkflowTaskDefinition = &WorkflowTaskDefinition{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowTaskDefinition = result
			return nil
		default:
...
func (src *WorkflowTaskDefinitionRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.WorkflowTaskDefinition != nil {
		return yaml.Marshal(&src.WorkflowTaskDefinition)
	}
