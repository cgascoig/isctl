...
func (dst *VnicFcQosPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "vnic.FcQosPolicy":
			var result *VnicFcQosPolicy = &VnicFcQosPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicFcQosPolicy = result
			return nil
		default:
...
func (src *VnicFcQosPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicFcQosPolicy != nil {
		return yaml.Marshal(&src.VnicFcQosPolicy)
	}
