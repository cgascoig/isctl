...
func (dst *StoragePureDiskResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.PureDisk.List":
			var result *StoragePureDiskList = &StoragePureDiskList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureDiskList = result
			return nil
		default:
...
func (src *StoragePureDiskResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StoragePureDiskList != nil {
		return yaml.Marshal(&src.StoragePureDiskList)
	}
