...
func (dst *PortMacBindingRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "port.MacBinding":
			var result *PortMacBinding = &PortMacBinding{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PortMacBinding = result
			return nil
		default:
...
func (src *PortMacBindingRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.PortMacBinding != nil {
		return yaml.Marshal(&src.PortMacBinding)
	}
