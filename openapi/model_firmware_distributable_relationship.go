...
func (dst *FirmwareDistributableRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "firmware.Distributable":
			var result *FirmwareDistributable = &FirmwareDistributable{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.FirmwareDistributable = result
			return nil
		case "mo.MoRef":
...
func (src *FirmwareDistributableRelationship) MarshalYAML() ([]byte, error) {
	if src.FirmwareDistributable != nil {
		return yaml.Marshal(&src.FirmwareDistributable)
	}
