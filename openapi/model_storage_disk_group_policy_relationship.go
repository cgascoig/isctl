...
func (dst *StorageDiskGroupPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.DiskGroupPolicy":
			var result *StorageDiskGroupPolicy = &StorageDiskGroupPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageDiskGroupPolicy = result
			return nil
		default:
...
func (src *StorageDiskGroupPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageDiskGroupPolicy != nil {
		return yaml.Marshal(&src.StorageDiskGroupPolicy)
	}
