...
func (dst *ResourceMembershipResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "resource.Membership.List":
			var result *ResourceMembershipList = &ResourceMembershipList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ResourceMembershipList = result
			return nil
		default:
...
func (src *ResourceMembershipResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.ResourceMembershipList != nil {
		return yaml.Marshal(&src.ResourceMembershipList)
	}
