...
func (dst *NiatelemetryApicFlashDetailsResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.ApicFlashDetails.List":
			var result *NiatelemetryApicFlashDetailsList = &NiatelemetryApicFlashDetailsList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryApicFlashDetailsList = result
			return nil
		default:
...
func (src *NiatelemetryApicFlashDetailsResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryApicFlashDetailsList != nil {
		return yaml.Marshal(&src.NiatelemetryApicFlashDetailsList)
	}
