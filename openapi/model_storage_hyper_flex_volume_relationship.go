...
func (dst *StorageHyperFlexVolumeRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.HyperFlexVolume":
			var result *StorageHyperFlexVolume = &StorageHyperFlexVolume{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHyperFlexVolume = result
			return nil
		default:
...
func (src *StorageHyperFlexVolumeRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageHyperFlexVolume != nil {
		return yaml.Marshal(&src.StorageHyperFlexVolume)
	}
