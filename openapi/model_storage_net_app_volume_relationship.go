...
func (dst *StorageNetAppVolumeRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.NetAppVolume":
			var result *StorageNetAppVolume = &StorageNetAppVolume{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppVolume = result
			return nil
		default:
...
func (src *StorageNetAppVolumeRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageNetAppVolume != nil {
		return yaml.Marshal(&src.StorageNetAppVolume)
	}
