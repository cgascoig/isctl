...
func (dst *SecurityUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "security.Unit.List":
			var result *SecurityUnitList = &SecurityUnitList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SecurityUnitList = result
			return nil
		default:
...
func (src *SecurityUnitResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.SecurityUnitList != nil {
		return yaml.Marshal(&src.SecurityUnitList)
	}
