...
func (dst *PortInterfaceBaseRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "port.InterfaceBase":
			var result *PortInterfaceBase = &PortInterfaceBase{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PortInterfaceBase = result
			return nil
		default:
...
func (src *PortInterfaceBaseRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.PortInterfaceBase != nil {
		return yaml.Marshal(&src.PortInterfaceBase)
	}
