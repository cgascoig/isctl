...
func (dst *StorageSasPortResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.SasPort.List":
			var result *StorageSasPortList = &StorageSasPortList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageSasPortList = result
			return nil
		default:
...
func (src *StorageSasPortResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageSasPortList != nil {
		return yaml.Marshal(&src.StorageSasPortList)
	}
