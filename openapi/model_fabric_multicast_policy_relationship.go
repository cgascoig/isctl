...
func (dst *FabricMulticastPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "fabric.MulticastPolicy":
			var result *FabricMulticastPolicy = &FabricMulticastPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.FabricMulticastPolicy = result
			return nil
		case "mo.MoRef":
...
func (src *FabricMulticastPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.FabricMulticastPolicy != nil {
		return yaml.Marshal(&src.FabricMulticastPolicy)
	}
