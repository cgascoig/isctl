...
func (dst *StoragePureHostRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.PureHost":
			var result *StoragePureHost = &StoragePureHost{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureHost = result
			return nil
		default:
...
func (src *StoragePureHostRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StoragePureHost != nil {
		return yaml.Marshal(&src.StoragePureHost)
	}
