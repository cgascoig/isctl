...
func (dst *RecoveryConfigResultRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "recovery.ConfigResult":
			var result *RecoveryConfigResult = &RecoveryConfigResult{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.RecoveryConfigResult = result
			return nil
		default:
...
func (src *RecoveryConfigResultRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.RecoveryConfigResult != nil {
		return yaml.Marshal(&src.RecoveryConfigResult)
	}
