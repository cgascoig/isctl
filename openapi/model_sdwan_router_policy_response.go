...
func (dst *SdwanRouterPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "sdwan.RouterPolicy.List":
			var result *SdwanRouterPolicyList = &SdwanRouterPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SdwanRouterPolicyList = result
			return nil
		default:
...
func (src *SdwanRouterPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.SdwanRouterPolicyList != nil {
		return yaml.Marshal(&src.SdwanRouterPolicyList)
	}
