...
func (dst *HyperflexNodeProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hyperflex.NodeProfile":
			var result *HyperflexNodeProfile = &HyperflexNodeProfile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexNodeProfile = result
			return nil
		case "mo.MoRef":
...
func (src *HyperflexNodeProfileRelationship) MarshalYAML() ([]byte, error) {
	if src.HyperflexNodeProfile != nil {
		return yaml.Marshal(&src.HyperflexNodeProfile)
	}
