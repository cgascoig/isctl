...
func (dst *NiaapiNiaMetadataResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "niaapi.NiaMetadata.List":
			var result *NiaapiNiaMetadataList = &NiaapiNiaMetadataList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiNiaMetadataList = result
			return nil
		default:
...
func (src *NiaapiNiaMetadataResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.NiaapiNiaMetadataList != nil {
		return yaml.Marshal(&src.NiaapiNiaMetadataList)
	}
