...
func (dst *StorageEnclosureDiskRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.EnclosureDisk":
			var result *StorageEnclosureDisk = &StorageEnclosureDisk{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageEnclosureDisk = result
			return nil
		default:
...
func (src *StorageEnclosureDiskRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageEnclosureDisk != nil {
		return yaml.Marshal(&src.StorageEnclosureDisk)
	}
