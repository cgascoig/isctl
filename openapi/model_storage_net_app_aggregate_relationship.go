...
func (dst *StorageNetAppAggregateRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.NetAppAggregate":
			var result *StorageNetAppAggregate = &StorageNetAppAggregate{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppAggregate = result
			return nil
		default:
...
func (src *StorageNetAppAggregateRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageNetAppAggregate != nil {
		return yaml.Marshal(&src.StorageNetAppAggregate)
	}
