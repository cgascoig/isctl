...
func (dst *OsDistributionRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "os.Distribution":
			var result *OsDistribution = &OsDistribution{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.OsDistribution = result
			return nil
		default:
...
func (src *OsDistributionRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.OsDistribution != nil {
		return yaml.Marshal(&src.OsDistribution)
	}
