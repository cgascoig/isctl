...
func (dst *SoftwareUcsdDistributableResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "software.UcsdDistributable.List":
			var result *SoftwareUcsdDistributableList = &SoftwareUcsdDistributableList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SoftwareUcsdDistributableList = result
			return nil
		default:
...
func (src *SoftwareUcsdDistributableResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.SoftwareUcsdDistributableList != nil {
		return yaml.Marshal(&src.SoftwareUcsdDistributableList)
	}
