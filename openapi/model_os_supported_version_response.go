...
func (dst *OsSupportedVersionResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "os.SupportedVersion.List":
			var result *OsSupportedVersionList = &OsSupportedVersionList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.OsSupportedVersionList = result
			return nil
		default:
...
func (src *OsSupportedVersionResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.OsSupportedVersionList != nil {
		return yaml.Marshal(&src.OsSupportedVersionList)
	}
