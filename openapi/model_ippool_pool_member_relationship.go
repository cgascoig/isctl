...
func (dst *IppoolPoolMemberRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "ippool.PoolMember":
			var result *IppoolPoolMember = &IppoolPoolMember{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IppoolPoolMember = result
			return nil
		case "mo.MoRef":
...
func (src *IppoolPoolMemberRelationship) MarshalYAML() ([]byte, error) {
	if src.IppoolPoolMember != nil {
		return yaml.Marshal(&src.IppoolPoolMember)
	}
