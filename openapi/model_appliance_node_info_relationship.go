...
func (dst *ApplianceNodeInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "appliance.NodeInfo":
			var result *ApplianceNodeInfo = &ApplianceNodeInfo{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ApplianceNodeInfo = result
			return nil
		case "mo.MoRef":
...
func (src *ApplianceNodeInfoRelationship) MarshalYAML() ([]byte, error) {
	if src.ApplianceNodeInfo != nil {
		return yaml.Marshal(&src.ApplianceNodeInfo)
	}
