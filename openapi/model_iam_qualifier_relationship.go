...
func (dst *IamQualifierRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.Qualifier":
			var result *IamQualifier = &IamQualifier{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamQualifier = result
			return nil
		case "mo.MoRef":
...
func (src *IamQualifierRelationship) MarshalYAML() ([]byte, error) {
	if src.IamQualifier != nil {
		return yaml.Marshal(&src.IamQualifier)
	}
