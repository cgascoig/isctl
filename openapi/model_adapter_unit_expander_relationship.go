...
func (dst *AdapterUnitExpanderRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "adapter.UnitExpander":
			var result *AdapterUnitExpander = &AdapterUnitExpander{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.AdapterUnitExpander = result
			return nil
		case "mo.MoRef":
...
func (src *AdapterUnitExpanderRelationship) MarshalYAML() ([]byte, error) {
	if src.AdapterUnitExpander != nil {
		return yaml.Marshal(&src.AdapterUnitExpander)
	}
