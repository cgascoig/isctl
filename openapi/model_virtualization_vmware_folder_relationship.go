...
func (dst *VirtualizationVmwareFolderRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "virtualization.VmwareFolder":
			var result *VirtualizationVmwareFolder = &VirtualizationVmwareFolder{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VirtualizationVmwareFolder = result
			return nil
		default:
...
func (src *VirtualizationVmwareFolderRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VirtualizationVmwareFolder != nil {
		return yaml.Marshal(&src.VirtualizationVmwareFolder)
	}
