...
func (dst *UuidpoolPoolMemberRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "uuidpool.PoolMember":
			var result *UuidpoolPoolMember = &UuidpoolPoolMember{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.UuidpoolPoolMember = result
			return nil
		default:
...
func (src *UuidpoolPoolMemberRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.UuidpoolPoolMember != nil {
		return yaml.Marshal(&src.UuidpoolPoolMember)
	}
