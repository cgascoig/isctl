...
func (dst *BootCddDeviceRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "boot.CddDevice":
			var result *BootCddDevice = &BootCddDevice{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.BootCddDevice = result
			return nil
		case "mo.MoRef":
...
func (src *BootCddDeviceRelationship) MarshalYAML() ([]byte, error) {
	if src.BootCddDevice != nil {
		return yaml.Marshal(&src.BootCddDevice)
	}
