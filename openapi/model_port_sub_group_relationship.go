...
func (dst *PortSubGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			return nil
		case "port.SubGroup":
			var result *PortSubGroup = &PortSubGroup{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PortSubGroup = result
			return nil
		default:
...
func (src *PortSubGroupRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.PortSubGroup != nil {
		return yaml.Marshal(&src.PortSubGroup)
	}
