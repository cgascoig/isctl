...
func (dst *StorageHitachiPoolResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.HitachiPool.List":
			var result *StorageHitachiPoolList = &StorageHitachiPoolList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHitachiPoolList = result
			return nil
		default:
...
func (src *StorageHitachiPoolResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageHitachiPoolList != nil {
		return yaml.Marshal(&src.StorageHitachiPoolList)
	}
