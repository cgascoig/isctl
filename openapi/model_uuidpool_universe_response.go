...
func (dst *UuidpoolUniverseResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "uuidpool.Universe.List":
			var result *UuidpoolUniverseList = &UuidpoolUniverseList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.UuidpoolUniverseList = result
			return nil
		default:
...
func (src *UuidpoolUniverseResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.UuidpoolUniverseList != nil {
		return yaml.Marshal(&src.UuidpoolUniverseList)
	}
