...
func (dst *PortGroupResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "port.Group.List":
			var result *PortGroupList = &PortGroupList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PortGroupList = result
			return nil
		default:
...
func (src *PortGroupResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.PortGroupList != nil {
		return yaml.Marshal(&src.PortGroupList)
	}
