...
func (dst *SearchSearchItemResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "search.SearchItem.List":
			var result *SearchSearchItemList = &SearchSearchItemList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SearchSearchItemList = result
			return nil
		default:
...
func (src *SearchSearchItemResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.SearchSearchItemList != nil {
		return yaml.Marshal(&src.SearchSearchItemList)
	}
