...
func (dst *VnicEthNetworkPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "vnic.EthNetworkPolicy":
			var result *VnicEthNetworkPolicy = &VnicEthNetworkPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicEthNetworkPolicy = result
			return nil
		default:
...
func (src *VnicEthNetworkPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicEthNetworkPolicy != nil {
		return yaml.Marshal(&src.VnicEthNetworkPolicy)
	}
