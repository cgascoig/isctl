...
func (dst *StorageNetAppInitiatorGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.NetAppInitiatorGroup":
			var result *StorageNetAppInitiatorGroup = &StorageNetAppInitiatorGroup{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppInitiatorGroup = result
			return nil
		default:
...
func (src *StorageNetAppInitiatorGroupRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageNetAppInitiatorGroup != nil {
		return yaml.Marshal(&src.StorageNetAppInitiatorGroup)
	}
