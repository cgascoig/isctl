...
func (dst *IamPermissionRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.Permission":
			var result *IamPermission = &IamPermission{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamPermission = result
			return nil
		case "mo.MoRef":
...
func (src *IamPermissionRelationship) MarshalYAML() ([]byte, error) {
	if src.IamPermission != nil {
		return yaml.Marshal(&src.IamPermission)
	}
