...
func (dst *StorageHitachiVolumeRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.HitachiVolume":
			var result *StorageHitachiVolume = &StorageHitachiVolume{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHitachiVolume = result
			return nil
		default:
...
func (src *StorageHitachiVolumeRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageHitachiVolume != nil {
		return yaml.Marshal(&src.StorageHitachiVolume)
	}
