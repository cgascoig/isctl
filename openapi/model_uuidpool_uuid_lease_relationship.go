...
func (dst *UuidpoolUuidLeaseRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "uuidpool.UuidLease":
			var result *UuidpoolUuidLease = &UuidpoolUuidLease{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.UuidpoolUuidLease = result
			return nil
		default:
...
func (src *UuidpoolUuidLeaseRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.UuidpoolUuidLease != nil {
		return yaml.Marshal(&src.UuidpoolUuidLease)
	}
