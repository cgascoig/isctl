...
func (dst *VnicFcNetworkPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "vnic.FcNetworkPolicy":
			var result *VnicFcNetworkPolicy = &VnicFcNetworkPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicFcNetworkPolicy = result
			return nil
		default:
...
func (src *VnicFcNetworkPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicFcNetworkPolicy != nil {
		return yaml.Marshal(&src.VnicFcNetworkPolicy)
	}
