...
func (dst *HyperflexClusterStoragePolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hyperflex.ClusterStoragePolicy":
			var result *HyperflexClusterStoragePolicy = &HyperflexClusterStoragePolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexClusterStoragePolicy = result
			return nil
		case "mo.MoRef":
...
func (src *HyperflexClusterStoragePolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.HyperflexClusterStoragePolicy != nil {
		return yaml.Marshal(&src.HyperflexClusterStoragePolicy)
	}
