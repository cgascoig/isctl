...
func (dst *StorageStoragePolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "storage.StoragePolicy":
			var result *StorageStoragePolicy = &StorageStoragePolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageStoragePolicy = result
			return nil
		default:
...
func (src *StorageStoragePolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageStoragePolicy != nil {
		return yaml.Marshal(&src.StorageStoragePolicy)
	}
