...
func (dst *StorageNetAppFcPortRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.NetAppFcPort":
			var result *StorageNetAppFcPort = &StorageNetAppFcPort{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppFcPort = result
			return nil
		default:
...
func (src *StorageNetAppFcPortRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageNetAppFcPort != nil {
		return yaml.Marshal(&src.StorageNetAppFcPort)
	}
