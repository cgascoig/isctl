...
func (dst *PciCoprocessorCardResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "pci.CoprocessorCard.List":
			var result *PciCoprocessorCardList = &PciCoprocessorCardList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PciCoprocessorCardList = result
			return nil
		default:
...
func (src *PciCoprocessorCardResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.PciCoprocessorCardList != nil {
		return yaml.Marshal(&src.PciCoprocessorCardList)
	}
