...
func (dst *StorageFlexUtilVirtualDriveResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.FlexUtilVirtualDrive.List":
			var result *StorageFlexUtilVirtualDriveList = &StorageFlexUtilVirtualDriveList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageFlexUtilVirtualDriveList = result
			return nil
		default:
...
func (src *StorageFlexUtilVirtualDriveResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageFlexUtilVirtualDriveList != nil {
		return yaml.Marshal(&src.StorageFlexUtilVirtualDriveList)
	}
