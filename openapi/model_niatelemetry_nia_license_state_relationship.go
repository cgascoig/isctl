...
func (dst *NiatelemetryNiaLicenseStateRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "niatelemetry.NiaLicenseState":
			var result *NiatelemetryNiaLicenseState = &NiatelemetryNiaLicenseState{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryNiaLicenseState = result
			return nil
		default:
...
func (src *NiatelemetryNiaLicenseStateRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.NiatelemetryNiaLicenseState != nil {
		return yaml.Marshal(&src.NiatelemetryNiaLicenseState)
	}
