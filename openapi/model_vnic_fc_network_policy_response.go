...
func (dst *VnicFcNetworkPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "vnic.FcNetworkPolicy.List":
			var result *VnicFcNetworkPolicyList = &VnicFcNetworkPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicFcNetworkPolicyList = result
			return nil
		default:
...
func (src *VnicFcNetworkPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VnicFcNetworkPolicyList != nil {
		return yaml.Marshal(&src.VnicFcNetworkPolicyList)
	}
