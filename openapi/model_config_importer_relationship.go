...
func (dst *ConfigImporterRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "config.Importer":
			var result *ConfigImporter = &ConfigImporter{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ConfigImporter = result
			return nil
		case "mo.MoRef":
...
func (src *ConfigImporterRelationship) MarshalYAML() ([]byte, error) {
	if src.ConfigImporter != nil {
		return yaml.Marshal(&src.ConfigImporter)
	}
