...
func (dst *NiatelemetryMsoEpgDetailsResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.MsoEpgDetails.List":
			var result *NiatelemetryMsoEpgDetailsList = &NiatelemetryMsoEpgDetailsList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryMsoEpgDetailsList = result
			return nil
		default:
...
func (src *NiatelemetryMsoEpgDetailsResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryMsoEpgDetailsList != nil {
		return yaml.Marshal(&src.NiatelemetryMsoEpgDetailsList)
	}
