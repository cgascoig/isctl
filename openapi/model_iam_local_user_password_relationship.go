...
func (dst *IamLocalUserPasswordRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.LocalUserPassword":
			var result *IamLocalUserPassword = &IamLocalUserPassword{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamLocalUserPassword = result
			return nil
		case "mo.MoRef":
...
func (src *IamLocalUserPasswordRelationship) MarshalYAML() ([]byte, error) {
	if src.IamLocalUserPassword != nil {
		return yaml.Marshal(&src.IamLocalUserPassword)
	}
