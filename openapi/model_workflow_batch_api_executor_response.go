...
func (dst *WorkflowBatchApiExecutorResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "workflow.BatchApiExecutor.List":
			var result *WorkflowBatchApiExecutorList = &WorkflowBatchApiExecutorList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowBatchApiExecutorList = result
			return nil
		default:
...
func (src *WorkflowBatchApiExecutorResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowBatchApiExecutorList != nil {
		return yaml.Marshal(&src.WorkflowBatchApiExecutorList)
	}
