...
func (dst *StorageNetAppEthernetPortRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.NetAppEthernetPort":
			var result *StorageNetAppEthernetPort = &StorageNetAppEthernetPort{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppEthernetPort = result
			return nil
		default:
...
func (src *StorageNetAppEthernetPortRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageNetAppEthernetPort != nil {
		return yaml.Marshal(&src.StorageNetAppEthernetPort)
	}
