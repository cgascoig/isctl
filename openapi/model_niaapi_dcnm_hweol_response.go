...
func (dst *NiaapiDcnmHweolResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niaapi.DcnmHweol.List":
			var result *NiaapiDcnmHweolList = &NiaapiDcnmHweolList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiDcnmHweolList = result
			return nil
		default:
...
func (src *NiaapiDcnmHweolResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiaapiDcnmHweolList != nil {
		return yaml.Marshal(&src.NiaapiDcnmHweolList)
	}
