...
func (dst *HyperflexNodeRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hyperflex.Node":
			var result *HyperflexNode = &HyperflexNode{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexNode = result
			return nil
		case "mo.MoRef":
...
func (src *HyperflexNodeRelationship) MarshalYAML() ([]byte, error) {
	if src.HyperflexNode != nil {
		return yaml.Marshal(&src.HyperflexNode)
	}
