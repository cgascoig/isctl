...
func (dst *StorageFlexUtilPhysicalDriveResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.FlexUtilPhysicalDrive.List":
			var result *StorageFlexUtilPhysicalDriveList = &StorageFlexUtilPhysicalDriveList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageFlexUtilPhysicalDriveList = result
			return nil
		default:
...
func (src *StorageFlexUtilPhysicalDriveResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageFlexUtilPhysicalDriveList != nil {
		return yaml.Marshal(&src.StorageFlexUtilPhysicalDriveList)
	}
