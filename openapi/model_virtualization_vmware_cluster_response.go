...
func (dst *VirtualizationVmwareClusterResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "virtualization.VmwareCluster.List":
			var result *VirtualizationVmwareClusterList = &VirtualizationVmwareClusterList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VirtualizationVmwareClusterList = result
			return nil
		default:
...
func (src *VirtualizationVmwareClusterResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VirtualizationVmwareClusterList != nil {
		return yaml.Marshal(&src.VirtualizationVmwareClusterList)
	}
