...
func (dst *KubernetesAciCniProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "kubernetes.AciCniProfile":
			var result *KubernetesAciCniProfile = &KubernetesAciCniProfile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.KubernetesAciCniProfile = result
			return nil
		case "mo.MoRef":
...
func (src *KubernetesAciCniProfileRelationship) MarshalYAML() ([]byte, error) {
	if src.KubernetesAciCniProfile != nil {
		return yaml.Marshal(&src.KubernetesAciCniProfile)
	}
