...
func (dst *PortMacBindingResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "port.MacBinding.List":
			var result *PortMacBindingList = &PortMacBindingList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PortMacBindingList = result
			return nil
		default:
...
func (src *PortMacBindingResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.PortMacBindingList != nil {
		return yaml.Marshal(&src.PortMacBindingList)
	}
