...
func (dst *SecurityUnitRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			return nil
		case "security.Unit":
			var result *SecurityUnit = &SecurityUnit{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SecurityUnit = result
			return nil
		default:
...
func (src *SecurityUnitRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.SecurityUnit != nil {
		return yaml.Marshal(&src.SecurityUnit)
	}
