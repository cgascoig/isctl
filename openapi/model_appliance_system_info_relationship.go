...
func (dst *ApplianceSystemInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "appliance.SystemInfo":
			var result *ApplianceSystemInfo = &ApplianceSystemInfo{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ApplianceSystemInfo = result
			return nil
		case "mo.MoRef":
...
func (src *ApplianceSystemInfoRelationship) MarshalYAML() ([]byte, error) {
	if src.ApplianceSystemInfo != nil {
		return yaml.Marshal(&src.ApplianceSystemInfo)
	}
