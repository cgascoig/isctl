...
func (dst *TamAdvisoryInfoResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "tam.AdvisoryInfo.List":
			var result *TamAdvisoryInfoList = &TamAdvisoryInfoList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.TamAdvisoryInfoList = result
			return nil
		default:
...
func (src *TamAdvisoryInfoResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.TamAdvisoryInfoList != nil {
		return yaml.Marshal(&src.TamAdvisoryInfoList)
	}
