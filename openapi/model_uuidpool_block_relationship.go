...
func (dst *UuidpoolBlockRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			return nil
		case "uuidpool.Block":
			var result *UuidpoolBlock = &UuidpoolBlock{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.UuidpoolBlock = result
			return nil
		default:
...
func (src *UuidpoolBlockRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.UuidpoolBlock != nil {
		return yaml.Marshal(&src.UuidpoolBlock)
	}
