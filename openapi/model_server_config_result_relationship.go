...
func (dst *ServerConfigResultRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "server.ConfigResult":
			var result *ServerConfigResult = &ServerConfigResult{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ServerConfigResult = result
			return nil
		default:
...
func (src *ServerConfigResultRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.ServerConfigResult != nil {
		return yaml.Marshal(&src.ServerConfigResult)
	}
