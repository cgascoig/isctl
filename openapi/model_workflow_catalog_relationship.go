...
func (dst *WorkflowCatalogRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "workflow.Catalog":
			var result *WorkflowCatalog = &WorkflowCatalog{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowCatalog = result
			return nil
		default:
...
func (src *WorkflowCatalogRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.WorkflowCatalog != nil {
		return yaml.Marshal(&src.WorkflowCatalog)
	}
