...
func (dst *VnicIscsiAdapterPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "vnic.IscsiAdapterPolicy.List":
			var result *VnicIscsiAdapterPolicyList = &VnicIscsiAdapterPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicIscsiAdapterPolicyList = result
			return nil
		default:
...
func (src *VnicIscsiAdapterPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VnicIscsiAdapterPolicyList != nil {
		return yaml.Marshal(&src.VnicIscsiAdapterPolicyList)
	}
