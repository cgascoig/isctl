...
func (dst *ResourceMembershipHolderRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "resource.MembershipHolder":
			var result *ResourceMembershipHolder = &ResourceMembershipHolder{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ResourceMembershipHolder = result
			return nil
		default:
...
func (src *ResourceMembershipHolderRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.ResourceMembershipHolder != nil {
		return yaml.Marshal(&src.ResourceMembershipHolder)
	}
