...
func (dst *StoragePureProtectionGroupResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.PureProtectionGroup.List":
			var result *StoragePureProtectionGroupList = &StoragePureProtectionGroupList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureProtectionGroupList = result
			return nil
		default:
...
func (src *StoragePureProtectionGroupResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StoragePureProtectionGroupList != nil {
		return yaml.Marshal(&src.StoragePureProtectionGroupList)
	}
