...
func (dst *ForecastInstanceRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "forecast.Instance":
			var result *ForecastInstance = &ForecastInstance{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ForecastInstance = result
			return nil
		case "mo.MoRef":
...
func (src *ForecastInstanceRelationship) MarshalYAML() ([]byte, error) {
	if src.ForecastInstance != nil {
		return yaml.Marshal(&src.ForecastInstance)
	}
