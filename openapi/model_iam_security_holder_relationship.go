...
func (dst *IamSecurityHolderRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.SecurityHolder":
			var result *IamSecurityHolder = &IamSecurityHolder{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamSecurityHolder = result
			return nil
		case "mo.MoRef":
...
func (src *IamSecurityHolderRelationship) MarshalYAML() ([]byte, error) {
	if src.IamSecurityHolder != nil {
		return yaml.Marshal(&src.IamSecurityHolder)
	}
