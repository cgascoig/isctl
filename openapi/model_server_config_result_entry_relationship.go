...
func (dst *ServerConfigResultEntryRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "server.ConfigResultEntry":
			var result *ServerConfigResultEntry = &ServerConfigResultEntry{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ServerConfigResultEntry = result
			return nil
		default:
...
func (src *ServerConfigResultEntryRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.ServerConfigResultEntry != nil {
		return yaml.Marshal(&src.ServerConfigResultEntry)
	}
