...
func (dst *EtherNetworkPortRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "ether.NetworkPort":
			var result *EtherNetworkPort = &EtherNetworkPort{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.EtherNetworkPort = result
			return nil
		case "mo.MoRef":
...
func (src *EtherNetworkPortRelationship) MarshalYAML() ([]byte, error) {
	if src.EtherNetworkPort != nil {
		return yaml.Marshal(&src.EtherNetworkPort)
	}
