...
func (dst *NetworkVlanPortInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "network.VlanPortInfo":
			var result *NetworkVlanPortInfo = &NetworkVlanPortInfo{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NetworkVlanPortInfo = result
			return nil
		default:
...
func (src *NetworkVlanPortInfoRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.NetworkVlanPortInfo != nil {
		return yaml.Marshal(&src.NetworkVlanPortInfo)
	}
