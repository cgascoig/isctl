...
func (dst *BootDeviceBootSecurityRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "boot.DeviceBootSecurity":
			var result *BootDeviceBootSecurity = &BootDeviceBootSecurity{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.BootDeviceBootSecurity = result
			return nil
		case "mo.MoRef":
...
func (src *BootDeviceBootSecurityRelationship) MarshalYAML() ([]byte, error) {
	if src.BootDeviceBootSecurity != nil {
		return yaml.Marshal(&src.BootDeviceBootSecurity)
	}
