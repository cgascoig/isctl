...
func (dst *StoragePhysicalDiskResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.PhysicalDisk.List":
			var result *StoragePhysicalDiskList = &StoragePhysicalDiskList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePhysicalDiskList = result
			return nil
		default:
...
func (src *StoragePhysicalDiskResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StoragePhysicalDiskList != nil {
		return yaml.Marshal(&src.StoragePhysicalDiskList)
	}
