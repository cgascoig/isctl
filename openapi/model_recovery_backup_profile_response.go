...
func (dst *RecoveryBackupProfileResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "recovery.BackupProfile.List":
			var result *RecoveryBackupProfileList = &RecoveryBackupProfileList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.RecoveryBackupProfileList = result
			return nil
		default:
...
func (src *RecoveryBackupProfileResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.RecoveryBackupProfileList != nil {
		return yaml.Marshal(&src.RecoveryBackupProfileList)
	}
