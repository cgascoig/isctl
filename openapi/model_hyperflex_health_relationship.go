...
func (dst *HyperflexHealthRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hyperflex.Health":
			var result *HyperflexHealth = &HyperflexHealth{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexHealth = result
			return nil
		case "mo.MoRef":
...
func (src *HyperflexHealthRelationship) MarshalYAML() ([]byte, error) {
	if src.HyperflexHealth != nil {
		return yaml.Marshal(&src.HyperflexHealth)
	}
