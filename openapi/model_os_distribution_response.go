...
func (dst *OsDistributionResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "os.Distribution.List":
			var result *OsDistributionList = &OsDistributionList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.OsDistributionList = result
			return nil
		default:
...
func (src *OsDistributionResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.OsDistributionList != nil {
		return yaml.Marshal(&src.OsDistributionList)
	}
