...
func (dst *OsConfigurationFileResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "os.ConfigurationFile.List":
			var result *OsConfigurationFileList = &OsConfigurationFileList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.OsConfigurationFileList = result
			return nil
		default:
...
func (src *OsConfigurationFileResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.OsConfigurationFileList != nil {
		return yaml.Marshal(&src.OsConfigurationFileList)
	}
