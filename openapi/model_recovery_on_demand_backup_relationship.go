...
func (dst *RecoveryOnDemandBackupRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "recovery.OnDemandBackup":
			var result *RecoveryOnDemandBackup = &RecoveryOnDemandBackup{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.RecoveryOnDemandBackup = result
			return nil
		default:
...
func (src *RecoveryOnDemandBackupRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.RecoveryOnDemandBackup != nil {
		return yaml.Marshal(&src.RecoveryOnDemandBackup)
	}
