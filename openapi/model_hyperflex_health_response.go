...
func (dst *HyperflexHealthResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		switch v {
		case "hyperflex.Health.List":
			var result *HyperflexHealthList = &HyperflexHealthList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexHealthList = result
			return nil
...
func (src *HyperflexHealthResponse) MarshalYAML() ([]byte, error) {
	if src.HyperflexHealthList != nil {
		return yaml.Marshal(&src.HyperflexHealthList)
	}
