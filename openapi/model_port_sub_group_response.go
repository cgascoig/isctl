...
func (dst *PortSubGroupResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "port.SubGroup.List":
			var result *PortSubGroupList = &PortSubGroupList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PortSubGroupList = result
			return nil
		default:
...
func (src *PortSubGroupResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.PortSubGroupList != nil {
		return yaml.Marshal(&src.PortSubGroupList)
	}
