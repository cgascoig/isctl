...
func (dst *RecommendationCapacityRunwayRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "recommendation.CapacityRunway":
			var result *RecommendationCapacityRunway = &RecommendationCapacityRunway{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.RecommendationCapacityRunway = result
			return nil
		default:
...
func (src *RecommendationCapacityRunwayRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.RecommendationCapacityRunway != nil {
		return yaml.Marshal(&src.RecommendationCapacityRunway)
	}
