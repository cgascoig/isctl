...
func (dst *ForecastCatalogRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "forecast.Catalog":
			var result *ForecastCatalog = &ForecastCatalog{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ForecastCatalog = result
			return nil
		case "mo.MoRef":
...
func (src *ForecastCatalogRelationship) MarshalYAML() ([]byte, error) {
	if src.ForecastCatalog != nil {
		return yaml.Marshal(&src.ForecastCatalog)
	}
