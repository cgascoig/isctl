...
func (dst *NiatelemetryNiaInventoryDcnmResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.NiaInventoryDcnm.List":
			var result *NiatelemetryNiaInventoryDcnmList = &NiatelemetryNiaInventoryDcnmList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryNiaInventoryDcnmList = result
			return nil
		default:
...
func (src *NiatelemetryNiaInventoryDcnmResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryNiaInventoryDcnmList != nil {
		return yaml.Marshal(&src.NiatelemetryNiaInventoryDcnmList)
	}
