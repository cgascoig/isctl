...
func (dst *PciDeviceResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "pci.Device.List":
			var result *PciDeviceList = &PciDeviceList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PciDeviceList = result
			return nil
		default:
...
func (src *PciDeviceResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.PciDeviceList != nil {
		return yaml.Marshal(&src.PciDeviceList)
	}
