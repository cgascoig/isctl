...
func (dst *VnicEthNetworkPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "vnic.EthNetworkPolicy.List":
			var result *VnicEthNetworkPolicyList = &VnicEthNetworkPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicEthNetworkPolicyList = result
			return nil
		default:
...
func (src *VnicEthNetworkPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VnicEthNetworkPolicyList != nil {
		return yaml.Marshal(&src.VnicEthNetworkPolicyList)
	}
