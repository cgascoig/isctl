...
func (dst *HyperflexLicenseRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hyperflex.License":
			var result *HyperflexLicense = &HyperflexLicense{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexLicense = result
			return nil
		case "mo.MoRef":
...
func (src *HyperflexLicenseRelationship) MarshalYAML() ([]byte, error) {
	if src.HyperflexLicense != nil {
		return yaml.Marshal(&src.HyperflexLicense)
	}
