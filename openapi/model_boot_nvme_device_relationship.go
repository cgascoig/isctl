...
func (dst *BootNvmeDeviceRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "boot.NvmeDevice":
			var result *BootNvmeDevice = &BootNvmeDevice{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.BootNvmeDevice = result
			return nil
		case "mo.MoRef":
...
func (src *BootNvmeDeviceRelationship) MarshalYAML() ([]byte, error) {
	if src.BootNvmeDevice != nil {
		return yaml.Marshal(&src.BootNvmeDevice)
	}
