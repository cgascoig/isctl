...
func (dst *WorkflowWorkflowMetadataRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "workflow.WorkflowMetadata":
			var result *WorkflowWorkflowMetadata = &WorkflowWorkflowMetadata{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowWorkflowMetadata = result
			return nil
		default:
...
func (src *WorkflowWorkflowMetadataRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.WorkflowWorkflowMetadata != nil {
		return yaml.Marshal(&src.WorkflowWorkflowMetadata)
	}
