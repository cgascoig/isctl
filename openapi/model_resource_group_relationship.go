...
func (dst *ResourceGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			return nil
		case "resource.Group":
			var result *ResourceGroup = &ResourceGroup{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ResourceGroup = result
			return nil
		default:
...
func (src *ResourceGroupRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.ResourceGroup != nil {
		return yaml.Marshal(&src.ResourceGroup)
	}
