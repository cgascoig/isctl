...
func (dst *TamBaseAdvisoryRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "tam.BaseAdvisory":
			var result *TamBaseAdvisory = &TamBaseAdvisory{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.TamBaseAdvisory = result
			return nil
		default:
...
func (src *TamBaseAdvisoryRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.TamBaseAdvisory != nil {
		return yaml.Marshal(&src.TamBaseAdvisory)
	}
