...
func (dst *StorageSasPortRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.SasPort":
			var result *StorageSasPort = &StorageSasPort{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageSasPort = result
			return nil
		default:
...
func (src *StorageSasPortRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageSasPort != nil {
		return yaml.Marshal(&src.StorageSasPort)
	}
