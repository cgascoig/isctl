...
func (dst *BiosBootDeviceRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "bios.BootDevice":
			var result *BiosBootDevice = &BiosBootDevice{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.BiosBootDevice = result
			return nil
		case "mo.MoRef":
...
func (src *BiosBootDeviceRelationship) MarshalYAML() ([]byte, error) {
	if src.BiosBootDevice != nil {
		return yaml.Marshal(&src.BiosBootDevice)
	}
