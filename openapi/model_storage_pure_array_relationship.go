...
func (dst *StoragePureArrayRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.PureArray":
			var result *StoragePureArray = &StoragePureArray{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureArray = result
			return nil
		default:
...
func (src *StoragePureArrayRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StoragePureArray != nil {
		return yaml.Marshal(&src.StoragePureArray)
	}
