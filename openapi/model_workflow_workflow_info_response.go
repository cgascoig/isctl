...
func (dst *WorkflowWorkflowInfoResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "workflow.WorkflowInfo.List":
			var result *WorkflowWorkflowInfoList = &WorkflowWorkflowInfoList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowWorkflowInfoList = result
			return nil
		default:
...
func (src *WorkflowWorkflowInfoResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowWorkflowInfoList != nil {
		return yaml.Marshal(&src.WorkflowWorkflowInfoList)
	}
