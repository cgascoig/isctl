...
func (dst *KubernetesNodeProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "kubernetes.NodeProfile":
			var result *KubernetesNodeProfile = &KubernetesNodeProfile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.KubernetesNodeProfile = result
			return nil
		case "mo.MoRef":
...
func (src *KubernetesNodeProfileRelationship) MarshalYAML() ([]byte, error) {
	if src.KubernetesNodeProfile != nil {
		return yaml.Marshal(&src.KubernetesNodeProfile)
	}
