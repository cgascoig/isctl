...
func (dst *VmrcConsoleResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "vmrc.Console.List":
			var result *VmrcConsoleList = &VmrcConsoleList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VmrcConsoleList = result
			return nil
		default:
...
func (src *VmrcConsoleResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VmrcConsoleList != nil {
		return yaml.Marshal(&src.VmrcConsoleList)
	}
