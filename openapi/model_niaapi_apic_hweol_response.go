...
func (dst *NiaapiApicHweolResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niaapi.ApicHweol.List":
			var result *NiaapiApicHweolList = &NiaapiApicHweolList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiApicHweolList = result
			return nil
		default:
...
func (src *NiaapiApicHweolResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiaapiApicHweolList != nil {
		return yaml.Marshal(&src.NiaapiApicHweolList)
	}
