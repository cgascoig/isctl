...
func (dst *NetworkFcZoneInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "network.FcZoneInfo":
			var result *NetworkFcZoneInfo = &NetworkFcZoneInfo{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NetworkFcZoneInfo = result
			return nil
		default:
...
func (src *NetworkFcZoneInfoRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.NetworkFcZoneInfo != nil {
		return yaml.Marshal(&src.NetworkFcZoneInfo)
	}
