...
// RproxyReverseProxy REST API endpoint for reverse proxy implementation. It receives an REST API request with URL and target type. Based on the target type, credentials is fetched frm the target prior to being forwarded to the right upstream endpoint.
