...
func (dst *NiatelemetryNexusDashboardsResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.NexusDashboards.List":
			var result *NiatelemetryNexusDashboardsList = &NiatelemetryNexusDashboardsList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryNexusDashboardsList = result
			return nil
		default:
...
func (src *NiatelemetryNexusDashboardsResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryNexusDashboardsList != nil {
		return yaml.Marshal(&src.NiatelemetryNexusDashboardsList)
	}
