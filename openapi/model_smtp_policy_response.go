...
func (dst *SmtpPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "smtp.Policy.List":
			var result *SmtpPolicyList = &SmtpPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SmtpPolicyList = result
			return nil
		default:
...
func (src *SmtpPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.SmtpPolicyList != nil {
		return yaml.Marshal(&src.SmtpPolicyList)
	}
