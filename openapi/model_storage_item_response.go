...
func (dst *StorageItemResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.Item.List":
			var result *StorageItemList = &StorageItemList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageItemList = result
			return nil
		default:
...
func (src *StorageItemResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageItemList != nil {
		return yaml.Marshal(&src.StorageItemList)
	}
