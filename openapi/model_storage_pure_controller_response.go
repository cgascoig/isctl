...
func (dst *StoragePureControllerResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.PureController.List":
			var result *StoragePureControllerList = &StoragePureControllerList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureControllerList = result
			return nil
		default:
...
func (src *StoragePureControllerResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StoragePureControllerList != nil {
		return yaml.Marshal(&src.StoragePureControllerList)
	}
