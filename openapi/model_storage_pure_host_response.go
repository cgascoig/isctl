...
func (dst *StoragePureHostResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "storage.PureHost.List":
			var result *StoragePureHostList = &StoragePureHostList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureHostList = result
			return nil
		default:
...
func (src *StoragePureHostResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.StoragePureHostList != nil {
		return yaml.Marshal(&src.StoragePureHostList)
	}
