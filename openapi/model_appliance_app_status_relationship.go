...
func (dst *ApplianceAppStatusRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "appliance.AppStatus":
			var result *ApplianceAppStatus = &ApplianceAppStatus{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ApplianceAppStatus = result
			return nil
		case "mo.MoRef":
...
func (src *ApplianceAppStatusRelationship) MarshalYAML() ([]byte, error) {
	if src.ApplianceAppStatus != nil {
		return yaml.Marshal(&src.ApplianceAppStatus)
	}
