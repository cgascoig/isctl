...
func (dst *VirtualizationVirtualDiskResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "virtualization.VirtualDisk.List":
			var result *VirtualizationVirtualDiskList = &VirtualizationVirtualDiskList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VirtualizationVirtualDiskList = result
			return nil
		default:
...
func (src *VirtualizationVirtualDiskResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VirtualizationVirtualDiskList != nil {
		return yaml.Marshal(&src.VirtualizationVirtualDiskList)
	}
