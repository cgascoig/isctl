...
func (dst *MemoryPersistentMemoryRegionRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "memory.PersistentMemoryRegion":
			var result *MemoryPersistentMemoryRegion = &MemoryPersistentMemoryRegion{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.MemoryPersistentMemoryRegion = result
			return nil
		case "mo.MoRef":
...
func (src *MemoryPersistentMemoryRegionRelationship) MarshalYAML() ([]byte, error) {
	if src.MemoryPersistentMemoryRegion != nil {
		return yaml.Marshal(&src.MemoryPersistentMemoryRegion)
	}
