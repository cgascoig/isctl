...
func (dst *EquipmentFanModuleRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "equipment.FanModule":
			var result *EquipmentFanModule = &EquipmentFanModule{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.EquipmentFanModule = result
			return nil
		case "mo.MoRef":
...
func (src *EquipmentFanModuleRelationship) MarshalYAML() ([]byte, error) {
	if src.EquipmentFanModule != nil {
		return yaml.Marshal(&src.EquipmentFanModule)
	}
