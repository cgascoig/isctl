...
func (dst *NiatelemetryMsoContractDetailsResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.MsoContractDetails.List":
			var result *NiatelemetryMsoContractDetailsList = &NiatelemetryMsoContractDetailsList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryMsoContractDetailsList = result
			return nil
		default:
...
func (src *NiatelemetryMsoContractDetailsResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryMsoContractDetailsList != nil {
		return yaml.Marshal(&src.NiatelemetryMsoContractDetailsList)
	}
