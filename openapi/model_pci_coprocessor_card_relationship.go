...
func (dst *PciCoprocessorCardRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "pci.CoprocessorCard":
			var result *PciCoprocessorCard = &PciCoprocessorCard{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PciCoprocessorCard = result
			return nil
		default:
...
func (src *PciCoprocessorCardRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.PciCoprocessorCard != nil {
		return yaml.Marshal(&src.PciCoprocessorCard)
	}
