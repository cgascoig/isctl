...
func (dst *WorkflowWorkflowInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "workflow.WorkflowInfo":
			var result *WorkflowWorkflowInfo = &WorkflowWorkflowInfo{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowWorkflowInfo = result
			return nil
		default:
...
func (src *WorkflowWorkflowInfoRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.WorkflowWorkflowInfo != nil {
		return yaml.Marshal(&src.WorkflowWorkflowInfo)
	}
