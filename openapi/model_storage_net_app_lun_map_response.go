...
func (dst *StorageNetAppLunMapResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.NetAppLunMap.List":
			var result *StorageNetAppLunMapList = &StorageNetAppLunMapList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppLunMapList = result
			return nil
		default:
...
func (src *StorageNetAppLunMapResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageNetAppLunMapList != nil {
		return yaml.Marshal(&src.StorageNetAppLunMapList)
	}
