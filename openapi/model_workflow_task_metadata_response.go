...
func (dst *WorkflowTaskMetadataResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "workflow.TaskMetadata.List":
			var result *WorkflowTaskMetadataList = &WorkflowTaskMetadataList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowTaskMetadataList = result
			return nil
		default:
...
func (src *WorkflowTaskMetadataResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowTaskMetadataList != nil {
		return yaml.Marshal(&src.WorkflowTaskMetadataList)
	}
