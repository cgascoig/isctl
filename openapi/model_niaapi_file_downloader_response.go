...
func (dst *NiaapiFileDownloaderResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niaapi.FileDownloader.List":
			var result *NiaapiFileDownloaderList = &NiaapiFileDownloaderList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiFileDownloaderList = result
			return nil
		default:
...
func (src *NiaapiFileDownloaderResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiaapiFileDownloaderList != nil {
		return yaml.Marshal(&src.NiaapiFileDownloaderList)
	}
