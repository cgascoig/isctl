...
func (dst *SdwanProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			return nil
		case "sdwan.Profile":
			var result *SdwanProfile = &SdwanProfile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SdwanProfile = result
			return nil
		default:
...
func (src *SdwanProfileRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.SdwanProfile != nil {
		return yaml.Marshal(&src.SdwanProfile)
	}
