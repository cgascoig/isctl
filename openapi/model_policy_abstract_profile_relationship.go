...
func (dst *PolicyAbstractProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "policy.AbstractProfile":
			var result *PolicyAbstractProfile = &PolicyAbstractProfile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.PolicyAbstractProfile = result
			return nil
		default:
...
func (src *PolicyAbstractProfileRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.PolicyAbstractProfile != nil {
		return yaml.Marshal(&src.PolicyAbstractProfile)
	}
