...
func (dst *NiatelemetryDcnmFanDetailsResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.DcnmFanDetails.List":
			var result *NiatelemetryDcnmFanDetailsList = &NiatelemetryDcnmFanDetailsList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryDcnmFanDetailsList = result
			return nil
		default:
...
func (src *NiatelemetryDcnmFanDetailsResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryDcnmFanDetailsList != nil {
		return yaml.Marshal(&src.NiatelemetryDcnmFanDetailsList)
	}
