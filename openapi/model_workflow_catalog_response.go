...
func (dst *WorkflowCatalogResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "workflow.Catalog.List":
			var result *WorkflowCatalogList = &WorkflowCatalogList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowCatalogList = result
			return nil
		default:
...
func (src *WorkflowCatalogResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowCatalogList != nil {
		return yaml.Marshal(&src.WorkflowCatalogList)
	}
