...
func (dst *VnicIscsiAdapterPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "vnic.IscsiAdapterPolicy":
			var result *VnicIscsiAdapterPolicy = &VnicIscsiAdapterPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicIscsiAdapterPolicy = result
			return nil
		default:
...
func (src *VnicIscsiAdapterPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicIscsiAdapterPolicy != nil {
		return yaml.Marshal(&src.VnicIscsiAdapterPolicy)
	}
