...
func (dst *ProcessorUnitRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			return nil
		case "processor.Unit":
			var result *ProcessorUnit = &ProcessorUnit{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ProcessorUnit = result
			return nil
		default:
...
func (src *ProcessorUnitRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.ProcessorUnit != nil {
		return yaml.Marshal(&src.ProcessorUnit)
	}
