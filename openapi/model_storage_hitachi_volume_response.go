...
func (dst *StorageHitachiVolumeResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.HitachiVolume.List":
			var result *StorageHitachiVolumeList = &StorageHitachiVolumeList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHitachiVolumeList = result
			return nil
		default:
...
func (src *StorageHitachiVolumeResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageHitachiVolumeList != nil {
		return yaml.Marshal(&src.StorageHitachiVolumeList)
	}
