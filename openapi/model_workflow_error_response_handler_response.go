...
func (dst *WorkflowErrorResponseHandlerResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "workflow.ErrorResponseHandler.List":
			var result *WorkflowErrorResponseHandlerList = &WorkflowErrorResponseHandlerList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowErrorResponseHandlerList = result
			return nil
		default:
...
func (src *WorkflowErrorResponseHandlerResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowErrorResponseHandlerList != nil {
		return yaml.Marshal(&src.WorkflowErrorResponseHandlerList)
	}
