...
func (dst *ProcessorUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.DocumentCount":
			...
			return nil
		case "processor.Unit.List":
			var result *ProcessorUnitList = &ProcessorUnitList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.ProcessorUnitList = result
			return nil
		default:
...
func (src *ProcessorUnitResponse) MarshalYAML() ([]byte, error) {
	if src.MoDocumentCount != nil {
		return yaml.Marshal(&src.MoDocumentCount)
	}

	if src.ProcessorUnitList != nil {
		return yaml.Marshal(&src.ProcessorUnitList)
	}
