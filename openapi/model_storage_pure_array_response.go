...
func (dst *StoragePureArrayResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.PureArray.List":
			var result *StoragePureArrayList = &StoragePureArrayList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureArrayList = result
			return nil
		default:
...
func (src *StoragePureArrayResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StoragePureArrayList != nil {
		return yaml.Marshal(&src.StoragePureArrayList)
	}
