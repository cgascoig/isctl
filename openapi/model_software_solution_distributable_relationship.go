...
func (dst *SoftwareSolutionDistributableRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "software.SolutionDistributable":
			var result *SoftwareSolutionDistributable = &SoftwareSolutionDistributable{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.SoftwareSolutionDistributable = result
			return nil
		default:
...
func (src *SoftwareSolutionDistributableRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.SoftwareSolutionDistributable != nil {
		return yaml.Marshal(&src.SoftwareSolutionDistributable)
	}
