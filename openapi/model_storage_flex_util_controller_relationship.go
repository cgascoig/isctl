...
func (dst *StorageFlexUtilControllerRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.FlexUtilController":
			var result *StorageFlexUtilController = &StorageFlexUtilController{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageFlexUtilController = result
			return nil
		default:
...
func (src *StorageFlexUtilControllerRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageFlexUtilController != nil {
		return yaml.Marshal(&src.StorageFlexUtilController)
	}
