...
func (dst *OsConfigurationFileRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "os.ConfigurationFile":
			var result *OsConfigurationFile = &OsConfigurationFile{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.OsConfigurationFile = result
			return nil
		default:
...
func (src *OsConfigurationFileRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.OsConfigurationFile != nil {
		return yaml.Marshal(&src.OsConfigurationFile)
	}
