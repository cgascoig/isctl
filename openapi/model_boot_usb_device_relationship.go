...
func (dst *BootUsbDeviceRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "boot.UsbDevice":
			var result *BootUsbDevice = &BootUsbDevice{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.BootUsbDevice = result
			return nil
		case "mo.MoRef":
...
func (src *BootUsbDeviceRelationship) MarshalYAML() ([]byte, error) {
	if src.BootUsbDevice != nil {
		return yaml.Marshal(&src.BootUsbDevice)
	}
