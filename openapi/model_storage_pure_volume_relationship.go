...
func (dst *StoragePureVolumeRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.PureVolume":
			var result *StoragePureVolume = &StoragePureVolume{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureVolume = result
			return nil
		default:
...
func (src *StoragePureVolumeRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StoragePureVolume != nil {
		return yaml.Marshal(&src.StoragePureVolume)
	}
