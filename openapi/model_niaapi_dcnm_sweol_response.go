...
func (dst *NiaapiDcnmSweolResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niaapi.DcnmSweol.List":
			var result *NiaapiDcnmSweolList = &NiaapiDcnmSweolList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiDcnmSweolList = result
			return nil
		default:
...
func (src *NiaapiDcnmSweolResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiaapiDcnmSweolList != nil {
		return yaml.Marshal(&src.NiaapiDcnmSweolList)
	}
