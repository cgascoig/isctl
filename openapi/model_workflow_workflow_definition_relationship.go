...
func (dst *WorkflowWorkflowDefinitionRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "workflow.WorkflowDefinition":
			var result *WorkflowWorkflowDefinition = &WorkflowWorkflowDefinition{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowWorkflowDefinition = result
			return nil
		default:
...
func (src *WorkflowWorkflowDefinitionRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.WorkflowWorkflowDefinition != nil {
		return yaml.Marshal(&src.WorkflowWorkflowDefinition)
	}
