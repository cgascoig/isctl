...
func (dst *RecommendationPhysicalItemRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "recommendation.PhysicalItem":
			var result *RecommendationPhysicalItem = &RecommendationPhysicalItem{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.RecommendationPhysicalItem = result
			return nil
		default:
...
func (src *RecommendationPhysicalItemRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.RecommendationPhysicalItem != nil {
		return yaml.Marshal(&src.RecommendationPhysicalItem)
	}
