...
func (dst *StorageDiskGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.DiskGroup":
			var result *StorageDiskGroup = &StorageDiskGroup{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageDiskGroup = result
			return nil
		default:
...
func (src *StorageDiskGroupRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageDiskGroup != nil {
		return yaml.Marshal(&src.StorageDiskGroup)
	}
