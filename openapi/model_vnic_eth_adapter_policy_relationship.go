...
func (dst *VnicEthAdapterPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "vnic.EthAdapterPolicy":
			var result *VnicEthAdapterPolicy = &VnicEthAdapterPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicEthAdapterPolicy = result
			return nil
		default:
...
func (src *VnicEthAdapterPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicEthAdapterPolicy != nil {
		return yaml.Marshal(&src.VnicEthAdapterPolicy)
	}
