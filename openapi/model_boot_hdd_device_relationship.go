...
func (dst *BootHddDeviceRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "boot.HddDevice":
			var result *BootHddDevice = &BootHddDevice{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.BootHddDevice = result
			return nil
		case "mo.MoRef":
...
func (src *BootHddDeviceRelationship) MarshalYAML() ([]byte, error) {
	if src.BootHddDevice != nil {
		return yaml.Marshal(&src.BootHddDevice)
	}
