...
func (dst *NiatelemetryNiaFeatureUsageResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niatelemetry.NiaFeatureUsage.List":
			var result *NiatelemetryNiaFeatureUsageList = &NiatelemetryNiaFeatureUsageList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiatelemetryNiaFeatureUsageList = result
			return nil
		default:
...
func (src *NiatelemetryNiaFeatureUsageResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiatelemetryNiaFeatureUsageList != nil {
		return yaml.Marshal(&src.NiatelemetryNiaFeatureUsageList)
	}
