...
func (dst *OprsDeploymentResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "oprs.Deployment.List":
			var result *OprsDeploymentList = &OprsDeploymentList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.OprsDeploymentList = result
			return nil
		default:
...
func (src *OprsDeploymentResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.OprsDeploymentList != nil {
		return yaml.Marshal(&src.OprsDeploymentList)
	}
