...
func (dst *VnicScpStatusResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "vnic.ScpStatus.List":
			var result *VnicScpStatusList = &VnicScpStatusList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicScpStatusList = result
			return nil
		default:
...
func (src *VnicScpStatusResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VnicScpStatusList != nil {
		return yaml.Marshal(&src.VnicScpStatusList)
	}
