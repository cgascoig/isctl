...
func (dst *StorageControllerRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "storage.Controller":
			var result *StorageController = &StorageController{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageController = result
			return nil
		default:
...
func (src *StorageControllerRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageController != nil {
		return yaml.Marshal(&src.StorageController)
	}
