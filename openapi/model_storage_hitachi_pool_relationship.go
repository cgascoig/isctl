...
func (dst *StorageHitachiPoolRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.HitachiPool":
			var result *StorageHitachiPool = &StorageHitachiPool{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHitachiPool = result
			return nil
		default:
...
func (src *StorageHitachiPoolRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageHitachiPool != nil {
		return yaml.Marshal(&src.StorageHitachiPool)
	}
