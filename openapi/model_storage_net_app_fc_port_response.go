...
func (dst *StorageNetAppFcPortResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.NetAppFcPort.List":
			var result *StorageNetAppFcPortList = &StorageNetAppFcPortList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppFcPortList = result
			return nil
		default:
...
func (src *StorageNetAppFcPortResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageNetAppFcPortList != nil {
		return yaml.Marshal(&src.StorageNetAppFcPortList)
	}
