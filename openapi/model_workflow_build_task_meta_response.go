...
func (dst *WorkflowBuildTaskMetaResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "workflow.BuildTaskMeta.List":
			var result *WorkflowBuildTaskMetaList = &WorkflowBuildTaskMetaList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowBuildTaskMetaList = result
			return nil
		default:
...
func (src *WorkflowBuildTaskMetaResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.WorkflowBuildTaskMetaList != nil {
		return yaml.Marshal(&src.WorkflowBuildTaskMetaList)
	}
