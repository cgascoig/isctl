...
func (dst *HyperflexServerModelRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hyperflex.ServerModel":
			var result *HyperflexServerModel = &HyperflexServerModel{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HyperflexServerModel = result
			return nil
		case "mo.MoRef":
...
func (src *HyperflexServerModelRelationship) MarshalYAML() ([]byte, error) {
	if src.HyperflexServerModel != nil {
		return yaml.Marshal(&src.HyperflexServerModel)
	}
