...
func (dst *StorageVdMemberEpRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.VdMemberEp":
			var result *StorageVdMemberEp = &StorageVdMemberEp{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageVdMemberEp = result
			return nil
		default:
...
func (src *StorageVdMemberEpRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageVdMemberEp != nil {
		return yaml.Marshal(&src.StorageVdMemberEp)
	}
