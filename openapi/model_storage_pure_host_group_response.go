...
func (dst *StoragePureHostGroupResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.PureHostGroup.List":
			var result *StoragePureHostGroupList = &StoragePureHostGroupList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePureHostGroupList = result
			return nil
		default:
...
func (src *StoragePureHostGroupResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StoragePureHostGroupList != nil {
		return yaml.Marshal(&src.StoragePureHostGroupList)
	}
