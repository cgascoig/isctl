...
func (dst *FabricEthNetworkControlPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "fabric.EthNetworkControlPolicy":
			var result *FabricEthNetworkControlPolicy = &FabricEthNetworkControlPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.FabricEthNetworkControlPolicy = result
			return nil
		case "mo.MoRef":
...
func (src *FabricEthNetworkControlPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.FabricEthNetworkControlPolicy != nil {
		return yaml.Marshal(&src.FabricEthNetworkControlPolicy)
	}
