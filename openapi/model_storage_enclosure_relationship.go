...
func (dst *StorageEnclosureRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "storage.Enclosure":
			var result *StorageEnclosure = &StorageEnclosure{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageEnclosure = result
			return nil
		default:
...
func (src *StorageEnclosureRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageEnclosure != nil {
		return yaml.Marshal(&src.StorageEnclosure)
	}
