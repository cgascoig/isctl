...
func (dst *WorkflowTaskInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "workflow.TaskInfo":
			var result *WorkflowTaskInfo = &WorkflowTaskInfo{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowTaskInfo = result
			return nil
		default:
...
func (src *WorkflowTaskInfoRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.WorkflowTaskInfo != nil {
		return yaml.Marshal(&src.WorkflowTaskInfo)
	}
