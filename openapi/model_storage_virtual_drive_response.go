...
func (dst *StorageVirtualDriveResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.VirtualDrive.List":
			var result *StorageVirtualDriveList = &StorageVirtualDriveList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageVirtualDriveList = result
			return nil
		default:
...
func (src *StorageVirtualDriveResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageVirtualDriveList != nil {
		return yaml.Marshal(&src.StorageVirtualDriveList)
	}
