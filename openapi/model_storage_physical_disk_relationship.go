...
func (dst *StoragePhysicalDiskRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.PhysicalDisk":
			var result *StoragePhysicalDisk = &StoragePhysicalDisk{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StoragePhysicalDisk = result
			return nil
		default:
...
func (src *StoragePhysicalDiskRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StoragePhysicalDisk != nil {
		return yaml.Marshal(&src.StoragePhysicalDisk)
	}
