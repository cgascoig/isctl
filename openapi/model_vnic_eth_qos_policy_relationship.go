...
func (dst *VnicEthQosPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "vnic.EthQosPolicy":
			var result *VnicEthQosPolicy = &VnicEthQosPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicEthQosPolicy = result
			return nil
		default:
...
func (src *VnicEthQosPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicEthQosPolicy != nil {
		return yaml.Marshal(&src.VnicEthQosPolicy)
	}
