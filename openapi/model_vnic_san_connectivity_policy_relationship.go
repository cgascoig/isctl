...
func (dst *VnicSanConnectivityPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "vnic.SanConnectivityPolicy":
			var result *VnicSanConnectivityPolicy = &VnicSanConnectivityPolicy{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicSanConnectivityPolicy = result
			return nil
		default:
...
func (src *VnicSanConnectivityPolicyRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.VnicSanConnectivityPolicy != nil {
		return yaml.Marshal(&src.VnicSanConnectivityPolicy)
	}
