...
func (dst *NiaapiApicCcoPostResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niaapi.ApicCcoPost.List":
			var result *NiaapiApicCcoPostList = &NiaapiApicCcoPostList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiApicCcoPostList = result
			return nil
		default:
...
func (src *NiaapiApicCcoPostResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiaapiApicCcoPostList != nil {
		return yaml.Marshal(&src.NiaapiApicCcoPostList)
	}
