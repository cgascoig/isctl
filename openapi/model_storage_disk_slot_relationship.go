...
func (dst *StorageDiskSlotRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.DiskSlot":
			var result *StorageDiskSlot = &StorageDiskSlot{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageDiskSlot = result
			return nil
		default:
...
func (src *StorageDiskSlotRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageDiskSlot != nil {
		return yaml.Marshal(&src.StorageDiskSlot)
	}
