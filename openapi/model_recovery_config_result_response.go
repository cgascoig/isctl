...
func (dst *RecoveryConfigResultResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "recovery.ConfigResult.List":
			var result *RecoveryConfigResultList = &RecoveryConfigResultList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.RecoveryConfigResultList = result
			return nil
		default:
...
func (src *RecoveryConfigResultResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.RecoveryConfigResultList != nil {
		return yaml.Marshal(&src.RecoveryConfigResultList)
	}
