...
func (dst *IamServiceProviderRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.ServiceProvider":
			var result *IamServiceProvider = &IamServiceProvider{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamServiceProvider = result
			return nil
		case "mo.MoRef":
...
func (src *IamServiceProviderRelationship) MarshalYAML() ([]byte, error) {
	if src.IamServiceProvider != nil {
		return yaml.Marshal(&src.IamServiceProvider)
	}
