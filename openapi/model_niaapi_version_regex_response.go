...
func (dst *NiaapiVersionRegexResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niaapi.VersionRegex.List":
			var result *NiaapiVersionRegexList = &NiaapiVersionRegexList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiVersionRegexList = result
			return nil
		default:
...
func (src *NiaapiVersionRegexResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiaapiVersionRegexList != nil {
		return yaml.Marshal(&src.NiaapiVersionRegexList)
	}
