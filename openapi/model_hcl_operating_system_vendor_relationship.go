...
func (dst *HclOperatingSystemVendorRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "hcl.OperatingSystemVendor":
			var result *HclOperatingSystemVendor = &HclOperatingSystemVendor{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.HclOperatingSystemVendor = result
			return nil
		case "mo.MoRef":
...
func (src *HclOperatingSystemVendorRelationship) MarshalYAML() ([]byte, error) {
	if src.HclOperatingSystemVendor != nil {
		return yaml.Marshal(&src.HclOperatingSystemVendor)
	}
