...
func (dst *IamIpAddressRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		switch v {
		case "iam.IpAddress":
			var result *IamIpAddress = &IamIpAddress{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.IamIpAddress = result
			return nil
		case "mo.MoRef":
...
func (src *IamIpAddressRelationship) MarshalYAML() ([]byte, error) {
	if src.IamIpAddress != nil {
		return yaml.Marshal(&src.IamIpAddress)
	}
