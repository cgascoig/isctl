...
func (dst *VnicLanConnectivityPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "vnic.LanConnectivityPolicy.List":
			var result *VnicLanConnectivityPolicyList = &VnicLanConnectivityPolicyList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.VnicLanConnectivityPolicyList = result
			return nil
		default:
...
func (src *VnicLanConnectivityPolicyResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.VnicLanConnectivityPolicyList != nil {
		return yaml.Marshal(&src.VnicLanConnectivityPolicyList)
	}
