...
func (dst *StorageHitachiParityGroupResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.HitachiParityGroup.List":
			var result *StorageHitachiParityGroupList = &StorageHitachiParityGroupList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHitachiParityGroupList = result
			return nil
		default:
...
func (src *StorageHitachiParityGroupResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageHitachiParityGroupList != nil {
		return yaml.Marshal(&src.StorageHitachiParityGroupList)
	}
