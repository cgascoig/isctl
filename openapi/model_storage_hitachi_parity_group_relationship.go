...
func (dst *StorageHitachiParityGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.HitachiParityGroup":
			var result *StorageHitachiParityGroup = &StorageHitachiParityGroup{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageHitachiParityGroup = result
			return nil
		default:
...
func (src *StorageHitachiParityGroupRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageHitachiParityGroup != nil {
		return yaml.Marshal(&src.StorageHitachiParityGroup)
	}
