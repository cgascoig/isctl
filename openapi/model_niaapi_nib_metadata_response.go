...
func (dst *NiaapiNibMetadataResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "niaapi.NibMetadata.List":
			var result *NiaapiNibMetadataList = &NiaapiNibMetadataList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.NiaapiNibMetadataList = result
			return nil
		default:
...
func (src *NiaapiNibMetadataResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.NiaapiNibMetadataList != nil {
		return yaml.Marshal(&src.NiaapiNibMetadataList)
	}
