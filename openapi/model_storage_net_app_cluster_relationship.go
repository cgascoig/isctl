...
func (dst *StorageNetAppClusterRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "storage.NetAppCluster":
			var result *StorageNetAppCluster = &StorageNetAppCluster{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageNetAppCluster = result
			return nil
		default:
...
func (src *StorageNetAppClusterRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.StorageNetAppCluster != nil {
		return yaml.Marshal(&src.StorageNetAppCluster)
	}
