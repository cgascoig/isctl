...
func (dst *StorageVirtualDriveContainerResponse) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ObjectType"]; ok {
		...
		case "mo.TagSummary":
			...
			return nil
		case "storage.VirtualDriveContainer.List":
			var result *StorageVirtualDriveContainerList = &StorageVirtualDriveContainerList{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.StorageVirtualDriveContainerList = result
			return nil
		default:
...
func (src *StorageVirtualDriveContainerResponse) MarshalYAML() ([]byte, error) {
	if src.MoAggregateTransform != nil {
	...
	if src.StorageVirtualDriveContainerList != nil {
		return yaml.Marshal(&src.StorageVirtualDriveContainerList)
	}
