...
func (dst *WorkflowErrorResponseHandlerRelationship) UnmarshalJSON(data []byte) error {
	...
	if v, ok := unmarshaled["ClassId"]; ok {
		...
		case "mo.MoRef":
			...
			dst.MoMoRef = result
			return nil
		case "workflow.ErrorResponseHandler":
			var result *WorkflowErrorResponseHandler = &WorkflowErrorResponseHandler{}
			err = json.Unmarshal(data, result)
			if err != nil {
				return err
			}
			dst.WorkflowErrorResponseHandler = result
			return nil
		default:
...
func (src *WorkflowErrorResponseHandlerRelationship) MarshalYAML() ([]byte, error) {
	if src.MoMoRef != nil {
		return yaml.Marshal(&src.MoMoRef)
	}

	if src.WorkflowErrorResponseHandler != nil {
		return yaml.Marshal(&src.WorkflowErrorResponseHandler)
	}
